...
namespace Chino_chan.Image
{
    ...
    public class Sankaku
    {
        ...
        public Sankaku(string Username, string Password)
        {
        ...
        public async Task<List<SankakuResponse>> GetImages(IEnumerable<string> Tags, int Limit = 20)
        {
            var Images = new List<SankakuResponse>();
            ...
            SendQuery(string.Join("+", _tags), out List<SankakuResponse> RawImages, Limit);

            foreach (var Image in RawImages)
            {
                var Img = Image;

                var Link = GetImageLink(Img.Url);

                var NameQuery = Link.Substring(Link.LastIndexOf('/')).Split('.', '?', '&');

                Stream Stream;
                do
                {
                    Stream = DownloadImage(Link, Image.Url, out bool Redirected, true);

                    if (Stream == null)
                    {
                        SendQuery(string.Join("+", _tags), out List<SankakuResponse> Imgs, 1);

                        if (Imgs[0].Url == Img.Url)
                            break;

                        Img = Imgs[0];
                        Link = GetImageLink(Img.Url);
                    }
                }
                while (Stream == null);
                ...
                if (Global.Imgur == null)
                {
                    ...
                    Url = File.Attachments.First().Url;
                }
                else
                {
                    Url = await Global.Imgur.UploadImage(Stream);
                }
                
                Images.Add(new SankakuResponse()
                {
                    Rating = Image.Rating,
                    Tags = Image.Tags,
                    Url = Url
        ...
        public async Task<SankakuResponse> GetImageAsync(IEnumerable<string> Tags)
        {
            var Images = await GetImages(Tags, 1);
            if (Images.Count == 0)
                return null;
            return Images[0];
        }
        ...
        private Stream DownloadImage(string ImageLink, string PostLink, out bool WasRedirected, bool ContainsVideo)
        {
            ...
            if (Response.ContentLength / 1024.0 / 1024.0 > 8192 && Global.Imgur == null)
            {
                return null;
            }
        ...
        private bool SendQuery(string Tag, out List<SankakuResponse> Results, int Limit = 20)
        {
            Results = new List<SankakuResponse>();
            try
            {
                ...
                foreach (Match Match in Regex.Matches(Content))
                {
                    try
                    {
                        ...
                        var Rating = "sfw";
                        if (Tags.Contains("rating:explicit") || Tags.Contains("rating:questionable"))
                            Rating = "nsfw";

                        Results.Add(new SankakuResponse()
                        {
                            Url = "https://chan.sankakucomplex.com/post/show/" + Match.Groups[1].Value,
                            Tags = Tags,
                            Rating = Rating
                        });
